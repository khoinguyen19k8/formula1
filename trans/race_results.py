...
results_racesDriversConstructorsCircuits_df = results_df.join(races_df, results_df["result_race_id"] == races_df["race_id"]) \
    ...
    .select(
        ...
        drivers_df["driver_id"], drivers_df["name"].alias("driver_name"), drivers_df["number"].alias("driver_number"), drivers_df["nationality"].alias("driver_nationality"),
...
merge_conditions = "tgt.driver_id = upd.driver_id AND tgt.race_id = upd.driver_id"
merge_delta_data(raceResults_report_df, "f1_presentation", "race_results", merge_conditions ,"race_id")
